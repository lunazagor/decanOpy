...
# #### Parse the arguments
# ##
parser = argparse.ArgumentParser('Mock Star Run')
#parser.add_argument('-d','--decan', nargs='+', type=str, required=True)
parser.add_argument('-s', '--star', required=True)
parser.add_argument('-yBC', '--yearBC', required=True)
parser.add_argument('-N', '--num', required=False, default="48")
parser.add_argument('-dOff', '--decOffset', required=False, default = "0") # decan offset in actual coordinates of star we are multiplying
...
if name == "None":
    name = "mockdata_" + str(num) + "x" + star + "-like_" + year + "BC"
...
(obj_list, hd_list) = mockCoords_StarLike(star, num, year, decOff)
