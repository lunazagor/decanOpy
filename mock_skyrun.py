...
tsave = time.localtime()
timestamp = time.strftime('%b-%d-%Y_%H%M', tsave) #get timestamp for unique savename

if name == "None":
    name = "mockdata_" + str(num) + "_" + year + "BC" + "-" + timestamp
...
(obj_list, hd_list, df) = mockCoords_randomStarField(num)
...
# save the generated stars 
df.to_csv(direct + "/ICs/star_data_" + timestamp + ".csv", index=False)
